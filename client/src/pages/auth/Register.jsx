...
const Field = ({ label, icon: Icon, type = 'text', showPasswordToggle = false, showPassword = false, onTogglePassword, error, ...props }) => (
  <div>
    ...
    <div className="relative">
      ...
      <input
        type={type}
        className={`w-full ${Icon ? 'pl-9' : 'pl-3'} ${showPasswordToggle ? 'pr-10' : 'pr-3'} py-2 text-sm border ${error ? 'border-danger' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors`}
        {...props}
      />
      {showPasswordToggle && (
        <button
...
const Register = () => {
  ...
  const handleEmailChange = (e) => {
    ...
    if (value && !value.toLowerCase().endsWith('@pec.edu.in')) {
      setEmailError('Please use your college email ending in @pec.edu.in');
    } else {
      setEmailError('');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.email.toLowerCase().endsWith('@pec.edu.in')) {
      setEmailError('Please use your college email ending in @pec.edu.in');
      return;
    }
